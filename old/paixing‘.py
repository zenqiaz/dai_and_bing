...
def find_onecode(pat, list):#如果一个听牌型可以被一个编码覆盖，那么找到所有满足条件的编码
    s = confirmpat(list, pat)
    res = []
    for i in refpats[0]:
        flag = 0
...
def are_one_code(list):#找到所有能被一个编码覆盖的听牌型
    res = []
    for i in list[2]:
...
def choosecode_many(ref, manypats, list):#上一个函数用很多次
    for i in manypats:
...
def check_s(ref, list):#找出一个解答在对应花色牌数少于13时不能满足条件的听牌型
    res = []
    for i in range(len(ref[1])):
...
def code_to_digits(code):#把编码转化为数字以便在excel里存储
    return code[0] * 100 + code[1] * 10 + code[2]

def pat_to_digits(pat):#把听牌型转化为数字以便在excel里存储
    res = 0
